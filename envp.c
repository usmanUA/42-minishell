...
char	*get_name_of_envp_variable(char *envp_string)
{
	char	*name;
	int		name_length;
...
char	*get_value_of_envp_variable(char *envp_string)
{
	char	*value;
	int		value_length;
	char	*value_string;
...
t_envp	*add_envp_name_and_value(t_envp *node, char *envp_string)
{
	node->env_name = get_name_of_envp_variable(envp_string);
	node->env_value = get_value_of_envp_variable(envp_string);
	return (node);
}
...
int	make_linked_list_of_envp(t_shell *data, char **envp)
{
	...
	while (envp[i])
	{
		...
		node = add_envp_name_and_value(node, envp[i]);
