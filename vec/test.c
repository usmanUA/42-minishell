...
void test_vec_from()
{
	...
	assert(memcmp(t1.mem, base, sizeof(base)) == 0);
	vec_free(&t1);
...
void test_vec_copy()
{
	...
	assert(memcmp(t2.mem, base, sizeof(base)) == 0);
	vec_free(&t1);
	vec_free(&t2);
...
void test_vec_push()
{
	...
	vec_push(&t1, &base[1]);
	vec_push(&t1, &base[3]);
	assert(memcmp(t1.mem, expect, sizeof(expect)) == 0);
	vec_free(&t1);
...
void test_vec_map()
{
	...
	assert(memcmp(t2.mem, expect, sizeof(expect)) == 0);
	vec_free(&t1);
	vec_free(&t2);
